...
function tkugp_categoryitem_html( $post) {
	...
	$favrt_item_default= array('on'=>1, 'title'=>"Show only Nicks's favorite tools:");
	if( empty($favrt_item) ){
...
function tkugp_items_content_html($ID, $item){
		...
		$item_html .= '<div class="tkugp_handlediv" title="Click to toggle"><br></div><h3 class="tkugp_hndle hndle"><span>Item: '.strip_tags($item['title']).'</span></h3>';
...
/**
* @11-mar-2107
* adding a WP_AJAX mapping to create/update POSTS from outside
*
*/
add_action('wp_ajax_tkugp_sync_guide' , 'tkugp_sync_guide');
add_action('wp_ajax_nopriv_tkugp_sync_guide' , 'tkugp_sync_guide');
function tkugp_sync_guide(){
	ob_clean();
	
	$wp_error = null;
	if ($_POST){
		
		//If the POST does not have title STOP
		if (empty($_POST['post_uber_title'])) {
		...
		$my_post = array(
		  ...
		  'post_title'    => sanitize_text_field( $_POST['post_uber_title'] ),
		  'post_type'	  => TKUGP_POST,
		  'post_status'   => 'draft',
		  'post_author'   => 1
		);
		
		$post_exists = post_exists($my_post["post_title"]);
		...
		if($post_exists){
			...
			//Insert the POST
			$post_id_insert = wp_insert_post( $my_post, $wp_error );
		...
		$post_id = $post_exists ?: $post_id_insert ;
		
		//hard coded sheet category columns names
		$sheet_cat_columns = array(
								0 => "FIRST_CATEGORY",
								1 => "SECOND_CATEGORY",
								2 => "THIRD_CATEGORY",
								3 => "FORTH_CATEGORY",
		...
		foreach ($sheet_cat_columns as $sheet_col_name){
			...
			if ($_POST[$sheet_col_name]) {
				...
				$cat_id = term_exists($sheet_col_name, TKUGP_POST . '_cat' );
				
				if( $cat_id == null) {
					
					//Add Categories if they do exists
					$cat_id = wp_insert_term($sheet_col_name, TKUGP_POST . '_cat');
					
					// Append the relationship
					wp_set_object_terms( $post_id, (int)$cat_id['term_id'], TKUGP_POST . '_cat', true ); // ture = append
				...
				$tags_array = explode(";",$_POST[$sheet_col_name]);
				$tag_id_array = array();
				...
				foreach ($tags_array as $tagname) {
					...
					if ($term == null) {
					$term = wp_insert_term($tagname, TKUGP_POST . '_tag');
					
					$tag_id_array[] = (int)$term['term_id'];
					
					wp_set_object_terms( $post_id, (int)$term['term_id'], TKUGP_POST . '_tag', true ); // ture = append
		...
		echo "Error: Request received no data.";
		wp_die();
	}
